...
describe('Web Server', () => {
  ...
  describe('HTTP Endpoints', () => {
    ...
    it('should serve built assets', async () => {
      const response = await fetch(managedTestServer.server.url)
      ...
      const jsResponse = await fetch(`${managedTestServer.server.url}/assets/${jsAsset}`)
      ...
      const cssResponse = await fetch(`${managedTestServer.server.url}/assets/${cssAsset}`)
    ...
    it('should return sessions list', async () => {
      const response = await fetch(`${managedTestServer.server.url}/api/sessions`)
      expect(response.status).toBe(200)
    ...
    it('should return individual session', async () => {
      ...
      const response = await fetch(
        `${managedTestServer.server.url}/api/sessions/${session.id}`
      )
      expect(response.status).toBe(200)
    ...
    it('should return 404 for non-existent session', async () => {
      ...
      const response = await fetch(
        `${managedTestServer.server.url}/api/sessions/${nonexistentId}`
      )
    ...
    it('should handle input to session', async () => {
      ...
      const response = await fetch(
        `${managedTestServer.server.url}/api/sessions/${session.id}/input`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    it('should return session output', async () => {
      ...
      const response = await fetch(
        `${managedTestServer.server.url}/api/sessions/${session.id}/buffer/raw`
      )
      expect(response.status).toBe(200)
    ...
    it('should return index.html for non-existent endpoints', async () => {
      const response = await fetch(`${managedTestServer.server.url}/api/nonexistent`)
      expect(response.status).toBe(200)
