...
describe('PTY Tools', () => {
  ...
  describe('ptySpawn', () => {
    beforeEach(() => {
      vi.spyOn(manager, 'spawn').mockImplementation((opts) => ({
        id: 'test-session-id',
  ...
  describe('ptyRead', () => {
    beforeEach(() => {
      ...
      vi.spyOn(manager, 'read').mockReturnValue({
        lines: ['line 1', 'line 2'],
        offset: 0,
        hasMore: false,
        totalLines: 2,
      })
      vi.spyOn(manager, 'search').mockReturnValue({
    ...
    it('should throw for invalid session', async () => {
      vi.spyOn(manager, 'get').mockReturnValue(null)
  ...
  describe('ptyList', () => {
    it('should list active sessions', async () => {
      ...
      vi.spyOn(manager, 'list').mockReturnValue(mockSessions)
